...
const AppointmentDetailsModal: React.FC<AppointmentDetailsModalProps> = ({
  ...
  const { patient, date, endDate, duration, doctor, service, treatment, notes, userId } = appointment;
  // Lógica de visibilidad
  const isAdmin = user && user.role === 'ADMIN';
  const isAssistant = user && user.role === 'ASSISTANT';
  const isDentistOwner = user && user.role === 'DENTIST' && user.id === userId;
        ...
        <div className="space-y-3 px-8 py-7">
          {canViewDetails ? (
            ...
            <div className="flex flex-col items-center justify-center py-10">
              <span className="text-lg font-semibold text-gray-700">Espacio ocupado</span>
              <span className="text-gray-400 text-sm">Horario bloqueado</span>
            </div>
          )}
